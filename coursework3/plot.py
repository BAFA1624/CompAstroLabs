...
for filename in os.listdir():
    if pattern.match(f"{filename}"):
        ...
        pl = ax1.plot(df["x"], df["d"], ls="-", linewidth=1, label=sim_type)[0]
        color = pl.get_color()
        ax2.plot(df["x"], df["v"], c=color, ls="-", linewidth=1, label=sim_type)
        ax3.plot(df["x"], df["p"], c=color, ls="-", linewidth=1, label=sim_type)
        ax4.plot(df["x"], df["e"], c=color, ls="-", linewidth=1, label=sim_type)
        if delete_files:
            os.remove(filename)
